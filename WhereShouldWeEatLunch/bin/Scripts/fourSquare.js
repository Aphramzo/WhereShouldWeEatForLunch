...
function loadFourSquareResults(categoryId) {
    ...
    $.getJSON('/eatery/FourSquareListByCoords', {
        ...
        var items = [];
        $('#eateryList').html = '';
        var linkString = '<li id="{0}"><a href="http://maps.google.com/?daddr={2},{3}">{1} ({4})</a></li>';
        $.each(data, function (key, val) {
            items.push(linkString.format(key,val.name,val.location.lat,val.location.lng,val.Distance));
        });
